...
def main():
    ...
    def histplot_boxplot(data, feature, bins=None, figsize=(12, 7)):
        ...
        sns.boxplot(data=data, x=feature, showmeans=True,
                    ax=ax_box, color="violet")
    ...
    def countplot(data, feature):
        ...
        for p in ax.patches:
            ...
            ax.annotate("{:.3g}%".format(100.*y/len(data)),
                        (x.mean(), y), ha="center", va="bottom")
        st.pyplot()

    plot = st.sidebar.selectbox(
        "Choose Plot", ('histograms', 'boxplot-histplot', 'countplot'))
    if plot == "histograms":
    ...
    Bivariate_plots = st.sidebar.selectbox(
        "Choose Bivariate Graph", ("Bivariate bar graph", "Scatterplots", "Heatmap "))

    # Bivariate -categorical vs numericAal

    def bivariate_barplot(data, feature1, feature2):
        data.groupby(feature1)[feature2].mean().plot(
